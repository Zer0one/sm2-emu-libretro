...
void Archive::transfer(void* data, usize num_bytes)
{
    ...
    if (m_failed || m_pos > m_in_size || num_bytes > m_in_size - m_pos) {
        m_failed = true;
...
void Archive::vector_pod(std::vector<u8>& v)
{
    ...
    if (loading()) {
        ...
        if (static_cast<usize>(len) > m_in_size - m_pos) {
            m_failed = true;
            v.clear();
            return;
        }
...
void Archive::deque_u32(std::deque<u32>& d)
{
    ...
    if (static_cast<usize>(len) > (m_in_size - m_pos) / sizeof(u32)) {
        m_failed = true;
        return;
    }
    for (u32 i = 0; i < len; ++i) {
...
bool read_header(const u8* data, usize size, Header& out, usize& payload_offset)
{
    ...
    // Leave room for the board field and reject corrupt lengths before they
    // can request an arbitrary allocation.
    if (game_len > size - ar.position()
        || size - ar.position() - game_len < sizeof(u32)) {
        SM2_ERROR("save-state: invalid game-name length");
        return false;
    }
