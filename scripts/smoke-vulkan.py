...
def main():
    ...
    shutdown=False;errors=[];pcm=bytearray();last=b'';frame=0;videos=0
    @ENV
    def env(cmd,data):
        nonlocal shutdown
    ...
    report={'frames':a.frames,'scale':a.scale,'context_cycle':a.context_cycle,'sync_masks':[7,15,7],
        'gpu_seconds':elapsed,'video_sha256':sha(last),'audio_sha256':sha(pcm),'nvram':files(out/'saves')}
    if a.reference:
